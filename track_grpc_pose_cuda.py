...
class ReIDFeatureExtractor:
    ...
    def extract_features(self, image):
        ...
        try:
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                ...
                gpu_resized = cv2.cuda.resize(gpu_img, self.input_size)
                resized = gpu_resized.download().astype(np.float32)
            else:
                resized = cv2.resize(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), self.input_size).astype(np.float32)
        except Exception as e:
            resized = cv2.resize(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), self.input_size).astype(np.float32)
    ...
    def fallback_feature_extractor(self, image):
        ...
        try:
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                hist = cv2.cuda.normalize(cv2.cuda_GpuMat(hist)).download()
            else:
...
class TargetState:
    ...
    def update_reid_features(self, frame, box, is_full_body=False):
        ...
        try:
            ...
            self.last_feature_save_time = current_time
        except Exception as e:
...
def calculate_3d_coordinates(depth_map, center_point, size=None):
    ...
    try:
        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
            ...
            gpu_roi.upload(depth_map[y1:y2, x1:x2].astype(np.float32))
            gpu_blur = cv2.cuda.medianBlur(gpu_roi, 5)
            depth_roi = gpu_blur.download().astype(np.uint16)
        else:
                ...
                if in_rgb is not None and in_depth is not None:
                    ...
                    try:
                        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                            ...
                            gpu_depth.upload(depth_frame.astype(np.float32))
                            gpu_blur = cv2.cuda.medianBlur(gpu_depth, 5)
                            depth_frame = gpu_blur.download().astype(np.uint16)
                        else:
