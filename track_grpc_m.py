...
class ReIDFeatureExtractor:
    # def __init__(self, model_path="baseline_R50.engine"):
        ...
        try:
            if target_state and target_state.active and target_state.world_position:
                ...
                coordinate_msg = tracking_pb2.CoordinateData(
                    ...
                    y=float(Y), 
                    z=float(Z),
                    distance=float(target_state.distance),
                    yaw=float(target_state.yaw),
                    pitch=float(target_state.pitch),
                    target_id=int(target_state.target_id),
                    confidence=float(target_state.confidence),
                    active=True,
                ...
                coordinate_msg = tracking_pb2.CoordinateData(
                    x=0.0, y=0.0, z=0.0,
                    distance=0.0, yaw=0.0, pitch=0.0,
                    target_id=0, confidence=0.0,
                    active=False,
                    timestamp=current_time
                )
...
class TargetState:
    ...
    def update_pose(self, landmarks, visibility, score):
        """更新目标的姿态信息"""
...
# 移除复杂的可视化绘制函数以提高性能
# def draw_custom_box(...) - 已被 draw_simple_box 替代
...
class MultiTargetManager:
    def __init__(self):
        ...
        self.initial_feature_duration = 5  # 减少特征收集时间
        # 移除姿态估计以提高性能
        self.pose = None
    ...
    def create_target(self, frame, box, cls_id, conf, depth_map):
        """创建新目标（优化：仅在非单目标模式或目标ID匹配时创建）"""
        # 如果处于单目标跟踪模式且已有跟随目标，不创建新目标
        if self.follow_only_mode and self.follow_target_id and self.follow_target_id in self.targets:
            return None
        ...
        print(f"📍 创建目标 ID: {target_id} 位置: ({center_x:.1f}, {center_y:.1f})")
        
        # 如果这是客户端指定的跟随目标，激活它并启用单目标模式
        if self.follow_target_id == target_id:
            self.set_active_target(target_id)
            self.follow_only_mode = True
            self.cleanup_non_follow_targets()
            if self.initial_feature_start is None:
                self.initial_feature_start = time.time()
                print(f"🎯 启动单目标跟踪模式：目标 ID: {target_id}")
    ...
    def find_best_match(self, frame, boxes, cls_ids, confs, depth_map):
        """优化的匹配算法：单目标模式下只匹配跟随目标"""
        matches = []
            ...
            if target and target.active:
                ...
                for i in range(len(boxes)):
                    ...
                    if target.position:
                        ...
                        match_score += 0.3 * position_score

                    if match_score > best_match_score and match_score > self.reappear_threshold:
            ...
            for target_id, target in self.targets.items():
                ...
                for i in range(len(boxes)):
                    ...
                    if target.position:
                        ...
                        match_score += 0.4 * position_score
    ...
    def update_inactive_targets(self):
        ...
        if self.follow_only_mode and self.follow_target_id:
            ...
            if target:
                if not target.active:
                    target.mark_lost()
                    if target.lock_strength < 0.3 or current_time - target.last_seen > 15.0:
                        print(f"移除丢失的跟随目标 ID: {self.follow_target_id}")
    ...
    def process_detections(self, frame, boxes, cls_ids, confs, depth_map):
        """优化的检测处理：单目标模式下仅处理跟随目标"""
        final_matches, unmatched_detections = self.find_best_match(frame, boxes, cls_ids, confs, depth_map)
        ...
        for target_id, detection_idx in final_matches.items():
            ...
            # 移除姿态估计以提高性能
            # self.estimate_pose(frame, target)
            
            # 如果是跟随目标，进行特征收集
            if target_id == self.follow_target_id:
                current_time = time.time()
                if self.initial_feature_start is not None and current_time - self.initial_feature_start <= self.initial_feature_duration:
                    if current_time - target.last_feature_save_time >= 1.0:  # 减少特征保存频率
        ...
        for detection_idx in unmatched_detections:
            ...
            if model.names[cls_id] == self.target_class and confs[detection_idx] > 0.6:
                ...
                if not self.follow_only_mode or (self.follow_target_id and len(self.targets) == 0):
                    ...
                    # 移除姿态估计
                    # if new_target:
                    #     self.estimate_pose(frame, new_target)
    ...
    # 移除姿态估计函数以提高性能
    # def estimate_pose(self, frame, target):
    #     """姿态估计已移除以提高性能"""
    #     pass
...
class ProcessingThread(threading.Thread):
        ...
        while not self.stop_event.is_set():
            ...
            if results and results[0].boxes is not None and len(results[0].boxes) > 0:
                ...
                final_matches, unmatched_detections = self.target_manager.process_detections(
                    frame, boxes_xyxy, cls_ids, confs, depth_frame
                )

                # 发送跟随目标数据到gRPC（单目标或多目标）
                follow_target = self.target_manager.get_follow_target()
                if follow_target and follow_target.active and self.grpc_enabled:
                    try:
    ...
    def draw_info(self, vis_frame, depth_frame):
        ...
        # 简化的控制说明
        cv2.putText(vis_frame, "Q=Quit | R=Reset | V=Toggle Vis", (10, vis_frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
def main(grpc_server='localhost:50051', enable_visualization=True):
    print(f"=== OAK单目标跟踪系统 (性能优化版) ===")
    print(f"gRPC服务器地址: {grpc_server}")
    print(f"可视化状态: {'启用' if enable_visualization else '禁用'}")
    # print(f"🚀 性能优化特性已启用:")
    # print(f"   - 单目标跟踪模式（减少多目标开销）")
    # print(f"   - 移除姿态估计（显著提升速度）")
    # print(f"   - 优化YOLO参数（416px输入，减少检测数量）")
    # print(f"   - 简化可视化绘制（减少GPU负载）")
    # print(f"   - 智能特征收集（仅跟随目标）")
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='OAK单目标跟踪系统 - 性能优化版 (集成gRPC通信)')
    ...
    print(f"=== OAK单目标跟踪系统启动 - 性能优化版 ===")
    print(f"gRPC服务器: {args.grpc_server}")
    print(f"最大重试次数: {args.retries}")
    print(f"可视化模式: {'启用' if enable_visualization else '禁用（高性能模式）'}")
    # print(f"🚀 性能优化功能:")
    # print(f"   ✅ 单目标跟踪（减少多目标计算开销）")
    # print(f"   ✅ 移除姿态估计（提升30-50% FPS）")
    # print(f"   ✅ 优化YOLO参数（416px输入，高置信度）")
    # print(f"   ✅ 简化可视化绘制（减少GPU负载）")
    # print(f"   ✅ 智能特征管理（仅针对跟随目标）")
    # print(f"   ✅ 减少内存分配和不必要计算")
