# 文件名: track_torch.py
# 描述: 自动选择中心目标，由gRPC指令或键盘'R'键触发，进行特征捕获后开始跟踪。
...
class TrackingGRPCClient:
        ...
        def coordinate_generator():
            while self.streaming:
                ...
                with self.coords_lock:
                    local_coords = self.latest_coords
    ...
    def update_target_coordinates(self, coords_tuple):
        if not self.connected or not self.streaming: return
        with self.coords_lock:
            self.latest_coords = coords_tuple
...
class ProcessingThread(threading.Thread):
    ...
    def handle_state(self, frame):
        ...
        if self.state == 'IDLE':
            ...
            if self.grpc_client and (time.time() - self.last_grpc_check_time > 0.1):
    ...
    def process_tracking(self, frame):
        ...
        if self.grpc_client:
            self.grpc_client.update_target_coordinates(self.last_coords)
